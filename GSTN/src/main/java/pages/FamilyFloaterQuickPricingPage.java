...
public class FamilyFloaterQuickPricingPage extends CustomAssert {
	...
	public void fillAddQuote(WebDriver driver, String testCaseName, XSSFWorkbook workbook, Connection conn,
		...
		selectFromDropdownByVisibleText(premiumFrequency, dataRow.getProperty("Premium Frequency"),
				"Premium Frequency");
		Thread.sleep(WaitTime.medium);
		...
		if (dataRow.getProperty("Product").equalsIgnoreCase("Activ Health (4212)")) {
				...
				if (key.equalsIgnoreCase(dataRow.getProperty("Relation"))) {
					...
					for (int x = 0; x < btnpressed; x++) {
						wait.until(ExpectedConditions
								.elementToBeClickable(driver.findElement(By.xpath("//input[@id='familysize']"))));
			...
			for (int x = 0; x < myList.size(); x++) {
				...
				if (myList.get(x).equalsIgnoreCase("Self")) {
					...
					if (dataRow.getProperty("SpouseDOB").equalsIgnoreCase("<1")) {
						...
						Thread.sleep(WaitTime.medium);
						clearAndSenKeys(driver.findElement(By.xpath("(//input[@id='Date of Birth'])[" + y + "]")),
								actualdate1, "Date of Birth");
						Thread.sleep(WaitTime.low);
			 ...
			 * netpremiumbefore.getText().toString().replace("â‚¹ ", "").replace(",", "");
			 * int netpremiumbeforevalNO = (int) Float.parseFloat(netpremiumbeforeval); int
...
//					ArrayList<String> doblist = new ArrayList<String>(Arrays.asList(dobval1.split("\\+")));
			...
			// ArrayList<String> roomlist = new
			// ArrayList<String>(Arrays.asList(roomval.split("\\+")));

			Thread.sleep(WaitTime.medium);
			// selectFromDropdownByVisibleTextStale(driver.findElement(By.xpath("(//input[@id='Date
			// of Birth']//following::select[3])[1]")),roomlist.get(0)," Room ");
			// Thread.sleep(WaitTime.low);
			...
			for (int x = 0; x < MemSize; x++) {
				...
				// WebElement MemberName = driver.findElement(By.xpath("(//input[@id='Member
				// Name'])["+y+"]"));
				WebElement MemberName = driver.findElement(By.xpath("(//input[@id='Member Name'])[" + y + "]"));
				WebElement dob = driver.findElement(By.xpath("(//input[@id='Date of Birth'])[" + y + "]"));
				WebElement gender = driver
...
//								Thread.sleep(WaitTime.low);
				...
				Thread.sleep(WaitTime.medium);
				// selectFromDropdownByVisibleTextStale(driver.findElement(By.xpath("(//input[@id='Member
				// Name']//following::select[2])["+y+"]")),deductlist.get(x)," deductlist ");
				// Thread.sleep(WaitTime.low);
...
//								driver.findElement(By.xpath("(//input[@id='Date of Birth'])["+y+"]")).sendKeys(Keys.TAB);
				...
				Thread.sleep(WaitTime.medium);
				selectFromDropdownByVisibleTextStale(
						driver.findElement(By.xpath("(//input[@id='Date of Birth']//following::select[1])[" + y + "]")),
						genderlist.get(x), " Gender ");
				Thread.sleep(WaitTime.low);

				// Relationship
				Thread.sleep(WaitTime.medium);
				selectFromDropdownByVisibleTextStale(
						driver.findElement(By.xpath("(//input[@id='Date of Birth']//following::select[2])[" + y + "]")),
						Relationlist.get(x), " Relationship ");
				Thread.sleep(WaitTime.low);
			}
...
//					Assert Quote Details
		...
		if (dataRow.getProperty("Product").equalsIgnoreCase("Activ Health (4212)")) {
			...
			for (int x = 0; x < isCoPaylist.size(); x++) {
				if (isCoPaylist.get(x).equalsIgnoreCase("Yes")) {
					int y = x + 1;
					Thread.sleep(WaitTime.medium);
					click(driver.findElement(By.xpath("(//input[@name='WMCP'])[" + y + "]")),
							" Hospital Cash Benefit checkBOX");
					Thread.sleep(WaitTime.medium);
			...
			if (dataRow.getProperty("OPDapplicable").equalsIgnoreCase("Yes")) {
				...
				Thread.sleep(WaitTime.medium);
				selectFromDropdownByVisibleText(opdeDropDown, dataRow.getProperty("OPDsi"),
						" OPD Expenses SumInsured ");
				Thread.sleep(WaitTime.medium);
			...
			if (dataRow.getProperty("HospitalCashBenefit").equalsIgnoreCase("Yes")) {
				...
				Thread.sleep(WaitTime.medium);
				selectFromDropdownByVisibleText(hcbDropDown, dataRow.getProperty("HCBsi"),
						" Hospital Cash Benefit  Expenses");
				Thread.sleep(WaitTime.medium);
			 ...
			 * =netpremiumafter.getText().toString().replace("₹‚¹ ","").replace(",", "");
			 * int premiumbeforeOPDNO = (int)Float.parseFloat(premiumbeforeOPD); int
		...
		if (dataRow.getProperty("Product").equalsIgnoreCase("POS Activ Assure (4220)")
				...
				|| dataRow.getProperty("Product").equalsIgnoreCase("Super Top Up (5223)")) {
...
//						wait.until(ExpectedConditions.elementToBeSelected(AffiliatedDiscount));
			selectFromDropdownByVisibleText(AffiliatedDiscount, dataRow.getProperty("AffiliatedEMPDiscount"),
...
//						Thread.sleep(WaitTime.medium);
			...
			if (dataRow.getProperty("Product").equalsIgnoreCase("POS Activ Assure (4227)")
					...
					|| dataRow.getProperty("Product").equalsIgnoreCase("Activ Care (4221)")
					|| dataRow.getProperty("Product").equalsIgnoreCase("Super Health Plus Top Up (4223)")
					|| dataRow.getProperty("Product").equalsIgnoreCase("Super Top Up (5223)")) {
				Thread.sleep(WaitTime.medium);
				selectFromDropdownByVisibleText(deductible, dataRow.getProperty("Deductible"), " Dedcutible ");
			...
			// selectFromDropdownByVisibleText(room, dataRow.getProperty("RoomCategory"),"
			// Room Category ");
...
//							         Thread.sleep(WaitTime.medium);
					...
					if (dataRow.getProperty("Gender").equalsIgnoreCase("Male")) {
						selectFromDropdownByVisibleText(gender, "Male", " Gender ");
						Thread.sleep(WaitTime.medium);
					} else if (dataRow.getProperty("Gender").equalsIgnoreCase("Others-Male")
							|| dataRow.getProperty("Gender").equalsIgnoreCase("Others-Female")) {
...
//							         Thread.sleep(WaitTime.medium);
					...
					if (dataRow.getProperty("Gender").equalsIgnoreCase("Male")
							|| dataRow.getProperty("Gender").equalsIgnoreCase("Others-Male")) {
						selectFromDropdownByVisibleText(
								driver.findElement(
										By.xpath("(//input[@id='Date of Birth']//following::select[1])[" + y + "]")),
								"Female", " Gender ");
						Thread.sleep(WaitTime.low);
					} else if (dataRow.getProperty("Gender").equalsIgnoreCase("Female")
							|| dataRow.getProperty("Gender").equalsIgnoreCase("Others-Female")) {
						selectFromDropdownByVisibleText(
								driver.findElement(
										By.xpath("(//input[@id='Date of Birth']//following::select[1])[" + y + "]")),
					...
					if (dataRow.getProperty("Kid1DOB").equalsIgnoreCase("<1")) {
						clearAndSenKeys(driver.findElement(By.xpath("(//input[@id='Age'])[" + y + "]")), "0", "Age");
						Thread.sleep(WaitTime.medium);
						String CollectAge = driver.findElement(By.xpath("(//input[@id='Age'])[" + y + "]"))
								.getAttribute("value");
						String[] arrofstr = acurrdate.split("/", 3);
						String date3 = arrofstr[2];
						int calactual = Integer.parseInt(date3);
...
//						Assert Quote Details
			...
			// value",netpremiumbeforeval,dataRow.getProperty("NetPremiumBeforeDiscouunt(BeforeOPD)"));

			// Accidental Hospitalization Booster
			if (dataRow.getProperty("Product").equalsIgnoreCase("POS Activ Assure (4227)")
					|| dataRow.getProperty("Product").equalsIgnoreCase("POS Activ Assure (4220)")) {
...
//							WebElement saveBTN1 = wait.until(ExpectedConditions.elementToBeClickable(By.id("btnSave")));	
	...
	public void AssertQuote(WebDriver driver, String testCaseName, XSSFWorkbook workbook, Connection conn,
			String stepGroup, CustomAssert customAssert) throws Exception {
		String sheetName = ConfigReader.getInstance().getValue(PropertyConfigs.TestSheet);
		Properties dataRow = ExcelRead.readRowDataInProperties(workbook, sheetName, testCaseName, stepGroup);
		...
		String premiuminclusiveofTAXval = premiuminclusiveofTAX.getText().toString().replace("₹ ", "").replace(",", "").replace(".00", "");
	...
	public void fillFamilyQuote(WebDriver driver, String testCaseName, XSSFWorkbook workbook, Connection conn,
			String stepGroup, CustomAssert customAssert) throws Exception {
